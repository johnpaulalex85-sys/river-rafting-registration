...
razorpay_client = razorpay.Client(
    auth=(
        os.environ.get("RAZORPAY_KEY_ID", "rzp_live_SNy6xvhqvYk7EL"),
        os.environ.get("RAZORPAY_KEY_SECRET", "W6WzBTLX836oswJu8eIiTdoO"),
    )
)
    ...
    return jsonify(
        {
            ...
            "razorpay_key_id": os.environ.get("RAZORPAY_KEY_ID", "rzp_live_SNy6xvhqvYk7EL"),
        }
    )
...
def verify_payment():
    ...
    # Send WhatsApp confirmation — non-blocking, failure does not affect payment response
    try:
        send_whatsapp_confirmation(updated)
    except Exception:
        logger.warning("WhatsApp notification failed for booking %s", updated["_id"], exc_info=True)
